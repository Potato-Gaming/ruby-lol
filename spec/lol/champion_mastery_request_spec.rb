...
describe ChampionMasteryRequest do
  ...
  describe "#total_score" do
    it "returns the total score" do
      stub_request_raw subject, 60, "scores/by-summoner/#{encrypted_summoner_id}"
      expect(subject.total_score summoner_id: encrypted_summoner_id).to eq 60
    end
  end

  describe "#find" do
    it "returns a ChampionMastery" do
      stub_request(subject, 'champion-mastery', "champion-masteries/by-summoner/#{encrypted_summoner_id}/by-champion/40")
      expect(subject.find 40, summoner_id: encrypted_summoner_id).to be_a DynamicModel
    end

    it "fetches ChampionMastery from the API" do
      stub_request(subject, 'champion-mastery', "champion-masteries/by-summoner/#{encrypted_summoner_id}/by-champion/40")
      result = subject.find 40, summoner_id: encrypted_summoner_id
      expect(result.highest_grade).to eq('S+')
      expect(result.champion_points).to eq(34356)
      expect(result.player_id).to eq(encrypted_summoner_id)
  ...
  describe "#all" do
    before { stub_request(subject, 'champion-masteries', "champion-masteries/by-summoner/#{encrypted_summoner_id}") }
    let(:result) { subject.all summoner_id: encrypted_summoner_id }
